    ...
    public static class ByteConverter
    {
        ...
        public static byte[] ToBytes(Type type, object obj)
        {
            ...
            if (type == typeof(DateTime))
            {
                ...
                TimeSpan span = dt - ByteConverter._startTime;
                return BitConverter.GetBytes((long)(span.TotalMilliseconds)); //BitConverter.GetBytes(dt.ToBinary());
            }
            if (type.IsEnum)
            {
                return BitConverter.GetBytes((int)obj);
        ...
        public static object Parse(Type type, byte[] buff, ref int offset)
        {
            ...
            if (type == typeof(DateTime))
            {
                long temp = BitConverter.ToInt64(buff, offset);
                offset += 8;
                return ByteConverter._startTime.AddMilliseconds(temp);// DateTime.FromBinary(temp);
            }
            if (type.IsEnum)
            {
                int temp = BitConverter.ToInt32(buff, offset);
